...
function FarmerDashboard() {
    ...
    const [myHireWorker, setMyHireWorker] = useState([]);
    const [myHireMachines, setMyHireMachines] = useState([]);
    const [myHireRequests, setMyHireRequests] = useState([]);

    const [hireWorkerCount, setHireWorkerCount] = useState(myHireWorker.length);
    const [hireMachineCount, setHireMachineCount] = useState(myHireMachines.length);

    const [farmerData, setFarmerData] = useState({
        stats: { activeRentals: hireMachineCount, workersHired: hireWorkerCount, totalSpent: 45000 },
        activeServices: [...myHireWorker, ...myHireMachines]
    });

    // console.log("My Machines:", myMachines);
    // console.log("My Hire Requests:", myHireRequests);
    // console.log("My Worker Hire Requests:", myHireWorker);
    // console.log("My Machine Hire Requests:", myHireMachines);
    ...
    useEffect(() => {
        ...
        const fetchMyMachine = async () => {
            try {
                const hireWorkerReq = await api.get(`/api/labor/worker-by-farmer/${userId}`);
                setMyHireWorker(hireWorkerReq?.data || []);
                setHireWorkerCount(hireWorkerReq?.data?.length || 0);


                const hireMachine = await api.get(`/api/machine-rental/farmer/${userId}`);
                setMyHireMachines(hireMachine?.data?.data || []);
                setHireMachineCount(hireMachine?.data?.data?.length || 0);

                setFarmerData(prev => ({ ...prev, stats: { ...prev.stats, workersHired: hireWorkerReq?.data?.length || 0, activeRentals: hireMachine?.data?.data?.length || 0 }
                    , activeServices : [...hireWorkerReq.data, ...hireMachine.data.data]
                }));
                // Fetch machines
                ...
                const hireRes = await api.get(
                    `/api/machine-rental/farmer/${userId}`
                );
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen bg-gray-50 dark:bg-gray-900 p-4 md:p-8">
            <div className="max-w-7xl mx-auto">
                    ...
                    <div className="lg:col-span-2">
                        ...
                        <div className="bg-indigo-600 rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-2xl shadow-indigo-200 dark:shadow-none">
                            <div className="relative z-10">
                                ...
                                <p className="opacity-80 mb-6 max-w-md">Browse available machines and verified labor groups in your area</p>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                    ...
                    <WorkerRequestPanel requests={myHireWorker} userMode="farmer" />
                </div>
