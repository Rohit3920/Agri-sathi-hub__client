...
import { Mail, Phone, MapPin, User, Settings, Briefcase, Calendar } from 'lucide-react';
...
const DetailItem = ({ label, value }) => (
    ...
    <div className="flex flex-col p-3 bg-gray-100 dark:bg-gray-700 rounded-lg shadow-inner">
        ...
        <p className="text-sm font-semibold text-gray-800 dark:text-gray-100 break-words mt-1">{value}</p>
    </div>
);

function ServicerProfile() {
    ...
    useEffect(() => {
        // Fetches user profile data from the API based on userId from URL params
    ...
    const handleViewMachine = () => {
        // Handles navigation to view machines/services provided by this servicer
    ...
    if (!userProfile) {
        return (
            <div className="flex flex-col justify-center items-center min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-700 dark:text-gray-300 text-lg">
                <p>Servicer profile not found.</p>
    ...
    return (
        <div className="min-h-screen w-full bg-gray-50 dark:bg-gray-900 py-8 px-4 sm:px-6 lg:px-8 flex justify-center items-start transition-colors duration-300">
            <div className="w-full max-w-5xl mx-auto bg-white dark:bg-gray-800 shadow-2xl rounded-2xl overflow-hidden md:flex border border-gray-200 dark:border-gray-700">
                ...
                <div className="md:w-1/3 p-8 bg-gradient-to-br from-indigo-600 to-blue-700 dark:from-indigo-800 dark:to-blue-900 text-white flex flex-col items-center relative text-center">

                    {/* Profile Image */}
                    <div className="relative mb-6">
                        <img
                            className="h-40 w-40 rounded-full object-cover border-4 border-white dark:border-gray-800 shadow-xl"
                            src={userProfile.profilePicture || 'https://via.placeholder.com/160/222222/FFFFFF?text=User'}
                            alt={`${userProfile.username}'s profile`}
                            // Fallback image handling
                            onError={(e) => { e.target.onerror = null; e.target.src = 'https://via.placeholder.com/160/222222/FFFFFF?text=User'; }}
                        />
                        {/* Role Badge */}
                        <span className="absolute bottom-0 right-0 px-3 py-1 bg-white dark:bg-gray-900 text-indigo-600 dark:text-indigo-400 text-xs font-bold rounded-full shadow-md capitalize border border-gray-300 dark:border-gray-600">
                    ...
                    <p className="text-indigo-200 dark:text-indigo-300 text-lg mb-4 capitalize">
                        Servicer Profile
                    </p>
                    ...
                    <div className="flex flex-col space-y-3 mt-4 w-full max-w-xs">
                        {isOwnProfile ? (
                            ...
                            <button
                                ...
                                className="flex items-center justify-center px-5 py-2 bg-white text-indigo-700 rounded-full font-semibold hover:bg-gray-100 transition-colors shadow-lg w-full"
                            >
                            ...
                            <>
                                <button
                                    ...
                                    className="flex items-center justify-center px-5 py-2 bg-white text-blue-600 rounded-full font-semibold hover:bg-blue-50 transition-colors shadow-lg w-full"
                                >
                                    <Mail className="w-5 h-5 mr-2" /> Message User
                                </button>
                                <button
                                    onClick={handleViewMachine}
                                    className="flex items-center justify-center px-5 py-2 bg-indigo-700 dark:bg-indigo-600 text-white rounded-full font-semibold border-2 border-white/50 hover:bg-indigo-800 transition-colors shadow-lg w-full"
                                >
                                    <Briefcase className="w-5 h-5 mr-2" /> View Services
                                </button>
                            </>
                        )}
                    </div>
                ...
                <div className="md:w-2/3 p-6 sm:p-8 space-y-8">
                    <h2 className="text-2xl font-extrabold text-gray-900 dark:text-white mb-6 border-b-2 border-indigo-500 pb-2">
                    ...
                    <section>
                        <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100 mb-4 flex items-center">
                            <User className="w-5 h-5 mr-2 text-indigo-500" /> Basic Information
                        </h3>
                    ...
                    {/* Description/Bio */}
                    <section className="p-4 bg-white dark:bg-gray-700 rounded-lg shadow border border-gray-200 dark:border-gray-600">
                        <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100 mb-2 flex items-center">
                            <Briefcase className="w-5 h-5 mr-2 text-blue-500" /> Bio / Summary
                        </h3>
                        <p className="text-gray-700 dark:text-gray-300 leading-relaxed text-sm">
                            {userProfile.description || 'The servicer has not yet provided a professional summary.'}
                        </p>
                    </section>
                            ...
                            <div className="space-y-4">
                                {userProfile.address.map((addr, index) => (
                                    ...
                                    <div key={index} className="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg shadow-sm border border-gray-200 dark:border-gray-600">
                                        <p className="font-bold text-indigo-600 dark:text-indigo-400 capitalize mb-1">
