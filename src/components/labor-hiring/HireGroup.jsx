...
const HireGroup = ({ groupId, farmerId, skills = [], onSuccess }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const endDateFormatted = new Date(new Date(form.startDate).getTime() + (form.days * 86400000)).toISOString().split('T')[0];
            ...
            alert("Group hire request sent successfully 👷‍♂️👷‍♂️");
            onSuccess?.();
        } catch (err) {
    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-[2.5rem] p-8 shadow-2xl border border-white/20">
            <h3 className="text-2xl font-black text-gray-900 dark:text-white mb-6 flex items-center gap-3">
                <CheckCircle className="text-indigo-600" /> Group Hiring Details
            </h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {skills.map((skill) => (
                            <button
                                ...
                                className={`px-4 py-2 rounded-xl text-sm font-bold border transition-all ${form.workType === skill && !customWork ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-gray-50 dark:bg-gray-700 border-gray-200 dark:border-gray-600'}`}
                            >
                                {skill}
                            </button>
                        ))}
                ...
                >
                    {loading ? "PROCESSING..." : "SEND GROUP HIRE REQUEST"}
                </button>
            </form>
        </div>
    );
